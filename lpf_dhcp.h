...
struct dhcp_option
{
    ...
    char hostname_buff[32];    /* Hostname buffer*/
    ...
    u_int8_t dhcp_release_flag;
    u_int8_t padding_flag;
    u_int8_t unused1[2];

    u_char dhopt_buff[512];

    u_int32_t dhopt_size;
...
struct dhcp_client
{
    ...
    int dhcp_packet_len;
    u_char dhcp_packet_send[PACK_BUFF_SIZE];
    u_char dhcp_packet_recv[PACK_BUFF_SIZE];
    u_char local_mac[ETHER_ADDR_LEN];
    u_char remote_mac[ETHER_ADDR_LEN];
    
    char iface_name[32];
    char ssid[64];

    struct ethernet_hdr *eth_hg;
    struct vlan_hdr *vlan_hg;
    struct iphdr *iph_g;
    struct udphdr *uh_g;
    struct dhcpv4_hdr *dhcph_g;
    u_int8_t *dhopt_pointer_g;
...
int lpf_recv_packet(int pktType, struct dhcp_client *c);
...
void set_option50(const char *ip_str, struct dhcp_option *option);   /* Sets the offer ip */
void set_option54(const char *ip_str, struct dhcp_option *option);    /* Sets the server ip */
void set_request_opt50_and_opt54(struct dhcp_client *client, const char *servIp); /* Sets the request ip and server ip */
